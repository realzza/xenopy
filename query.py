...
class Query:
    ...
    def retrieve_meta(self, recordings_only=False, verbose=False, attempts=10):
        ...
        while page < page_num + 1:
            url = 'https://www.xeno-canto.org/api/2/recordings?query={0}&page={1}'.format(query_content, page)
            n_attempts = 0
    ...
    def retrieve_recordings(self, multiprocess=False, nproc=5, attempts=10, outdir="datasets/"):
        """
        params:
    ...
    def __multi_dl(self, nproc=5, attempts=10, outdir="datasets/"):
        ...
        for i in range(nproc):
            p = Process(target=self.__single_dl, args=(i, recording_packs[i], attempts, outdir))
            p.start()
